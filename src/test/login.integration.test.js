...
integrationTest('Authenticates user with correct details', async (t) => {
  try {
    await saveUser()
...
integrationTest('Does not authenticate user with incorrect email', async (t) => {
  try {
    await saveUser()
    t.pass('Succesfully saved user record')
...
integrationTest('Does not authenticate user with incorrect password', async (t) => {
  try {
    await saveUser()
    t.pass('Succesfully saved user record')
